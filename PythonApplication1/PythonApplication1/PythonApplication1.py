# -*- coding: utf-8 -*-
"""
完整功能 + 效能優化版 v14 (多模型 + 均線視覺化 + GEX 強化 + 作戰地圖)
- 台股 Yahoo 連結改為台灣站點
- 均線視覺化 + 多空排列 Badge
- 三個 AI 模型 (RF/XGB/LGB) 訓練與預測
- GEX 強化：診斷、降級、正負總量
- GEX 篩選器 (gex_screener)
- 新增：個股價格警戒線區間圖 + 戰術建議
- 新增：盤前戰術板 (/battle_map)
- 新增：情境模擬器 (手動輸入股價模擬 GEX 壓力)
- 修正：NoneType 比較錯誤、表格對齊、戰術卡片美化
"""
import os
import sys
import json
...
# --- 暴力修正版：全局強制對齊 ---
# 這段代碼會幫所有表格強制加上樣式，您不需要修改原本的 render_template_string
@app.after_request
def add_table_style(response):
    if response.content_type == 'text/html; charset=utf-8':
        content = response.get_data(as_text=True)
        # 強制把所有的 <table> 變成自適應寬度，且不會撐開版面
        content = content.replace("<table", "<div style='overflow-x: auto; width: 100%;'><table style='width: 100%; min-width: 500px;'")
...
DEFAULT_SETTINGS = {
    ...
    "cache_ttl_minutes": 5, "max_loss_per_trade": 5.0, "max_loss_per_day": 10.0,
...
for f in [STOCK_FILE_TW, STOCK_FILE_US]:
    if not os.path.exists(f):
        with open(f, "w", encoding="utf-8") as _: pass
if not os.path.exists(POSITIONS_FILE):
    with open(POSITIONS_FILE, "w", encoding='utf-8') as f: json.dump([], f)
if not os.path.exists(WATCHLIST_FILE):
    with open(WATCHLIST_FILE, "w", encoding='utf-8') as f: json.dump(["NVDA","TSLA","AAPL","MSFT","GOOGL"], f)
...
def set_cached_gex(ticker, data):
    with _gex_lock:
        _gex_cache[ticker] = {'data': data, 'time': time.time()}
...
def get_rf_model(ticker, force_retrain=False):
    ...
    if model:
        with _model_lock_rf:
            _model_cache_rf[cache_key] = (model, scaler, now)
    return model, scaler
...
def get_xgb_model(ticker, force_retrain=False):
    ...
    if model:
        with _model_lock_xgb:
            _model_cache_xgb[cache_key] = (model, scaler, now)
    return model, scaler
...
def get_lgb_model(ticker, force_retrain=False):
    ...
    if model:
        with _model_lock_lgb:
            _model_cache_lgb[cache_key] = (model, scaler, now)
    return model, scaler
...
def get_growth_score(ticker, df):
    ...
    try:
        ...
        if any(k in sector for k in ["AI","SEMICONDUCTOR","CLOUD","NUCLEAR","ROBOTICS"]):
            score+=20
    except: pass
    return min(100, score), []
...
def get_ten_bagger_score(ticker):
    ...
    try:
        ...
        if any(x in ticker for x in ["NVDA","AMD","AVGO","MRVL","VRT","SMCI","PLTR","OKLO","IONQ"]):
            score+=30
    except: pass
    return min(100, score)
...
def ensemble_predict(ticker, df):
    ...
    growth, growth_details = get_growth_score(ticker, df)
    scores['GROWTH'] = growth
    details['GROWTH'] = f"成長 {growth:.0f}"
    
    tenbagger = get_ten_bagger_score(ticker)
    
    if not scores:
        return None, None, {}, None, None, None, None, None, None, None, None
    
    weights = {
        'RF': 0.25, 'XGB': 0.25, 'LGB': 0.20,
        'SMART': 0.10, 'TREND': 0.10, 'GROWTH': 0.10
    }
    ...
    consensus = final
    return final, signal, details, 0.5, smart, hype, trend, growth, tenbagger, growth_details, consensus
...
def calculate_gamma_exposure(ticker, current_price=None, r=0.05):
    if '.TW' not in ticker and not is_us_market_open():
    ...
    try:
        ...
        result = {
            ...
            "y_range": y_range,
            "method": "Black-Scholes Gamma + 多到期日加總"
        }
        
        set_cached_gex(ticker, result)
        logger.info(f"✅ {ticker} GEX 計算成功，Call Wall: {call_wall}, Put Wall: {put_wall}")
        return result
    except Exception as e:
...
def get_options_signals(ticker):
    ...
    if not pcr and not mp and not iv and not gex:
        return None
    ...
    if gex:
        if gex['call_wall'] and gex['current_price'] > gex['call_wall']:
            score-=1.5; reasons.append(f"高於 Call Wall ${gex['call_wall']:.2f}")
        if gex['put_wall'] and gex['current_price'] < gex['put_wall']:
            score+=1.5; reasons.append(f"低於 Put Wall ${gex['put_wall']:.2f}")
        if gex['gamma_flip_strike'] and gex['current_price'] > gex['gamma_flip_strike']:
            score+=1; reasons.append(f"站上 Gamma Flip ${gex['gamma_flip_strike']:.2f}")
    ...
    return {
        "pcr": pcr, "max_pain": mp, "iv": iv, "gex": gex,
        "composite": {"score": round(score,2), "text": composite, "color": color, "reasons": reasons}
    }
...
def update_tenbagger():
    ...
    for market in ['tw','us']:
        for ticker in get_all_tickers(market):
            ...
            if radar and radar.get('score', 0) >= 3:
    ...
    try:
        ...
        try:
            ...
            if not qf.empty and 'Total Revenue' in qf.index:
                rev = qf.loc['Total Revenue'].dropna()
                if len(rev)>=2:
                    revenue_growth = (rev.iloc[0]-rev.iloc[1])/rev.iloc[1]*100
        except: pass
        gm_growth = 0
        try:
...
US_SECTOR_ETFS = {
    "AI": "AIQ", "電力": "XLU", "機器人": "ARKQ", "核能": "URA", "網路安全": "CIBR"
}
TW_SECTOR_ETFS = {
    "半導體": "0050.TW", "電子": "0052.TW", "金融": "0055.TW"
}
...
def get_tw_index():
    try:
        df = yf.Ticker("^TWII").history(period="2d")
        if not df.empty:
            curr, prev = df['Close'].iloc[-1], df['Close'].iloc[-2]
            change, pct = curr-prev, (curr-prev)/prev*100
...
def get_otc_index():
    try:
        df = yf.Ticker("^TWOII").history(period="2d")
        # 只要資料抓到了，就回傳數字；抓不到就一律回傳 0
        if not df.empty and len(df) >= 2:
            curr, prev = df['Close'].iloc[-1], df['Close'].iloc[-2]
            return float(curr), float(curr - prev), float((curr - prev)/prev*100)
    except:
        pass
    return 0.0, 0.0, 0.0
def get_dow_index():
    try:
        df = yf.Ticker("^DJI").history(period="2d")
        if not df.empty:
            curr, prev = df['Close'].iloc[-1], df['Close'].iloc[-2]
            change, pct = curr-prev, (curr-prev)/prev*100
            return round(curr,2), round(change,2), round(pct,2)
...
def get_nasdaq_index():
    try:
        df = yf.Ticker("^IXIC").history(period="2d")
        if not df.empty:
            curr, prev = df['Close'].iloc[-1], df['Close'].iloc[-2]
            change, pct = curr-prev, (curr-prev)/prev*100
            return round(curr,2), round(change,2), round(pct,2)
...
def get_phlx_index():
    try:
        df = yf.Ticker("^SOX").history(period="2d")
        if not df.empty:
            curr, prev = df['Close'].iloc[-1], df['Close'].iloc[-2]
            change, pct = curr-prev, (curr-prev)/prev*100
            return round(curr,2), round(change,2), round(pct,2)
...
def get_yield_curve_inversion():
    try:
        tnx = yf.Ticker("^TNX")
        fvx = yf.Ticker("^FVX")
        df_tnx = tnx.history(period="1d")
        df_fvx = fvx.history(period="1d")
        if df_tnx.empty or df_fvx.empty:
            return None, None, "無法取得殖利率資料"
        tnx_rate = df_tnx['Close'].iloc[-1]
        fvx_rate = df_fvx['Close'].iloc[-1]
...
def update_all_gex_screener():
    ...
    for ticker in tickers:
        try:
            ...
            results.append({
                "ticker": ticker,
                "current_price": gex.get("current_price"),
                "net_gex": gex.get("net_total_gex"),
                "positive_gex": gex.get("total_positive_gex"),
                "negative_gex": gex.get("total_negative_gex"),
                "call_wall": gex.get("call_wall"),
                "put_wall": gex.get("put_wall"),
                "gamma_flip": gex.get("gamma_flip_strike"),
                "last_update": time.time()
            })
            time.sleep(0.2)
        except Exception as e:
            logger.error(f"GEX 篩選器更新 {ticker} 失敗: {e}")
...
# ================== 輔助函數：產生戰術建議 (防 None) ==================
def generate_tactical_advice(current_price, call_wall, put_wall, gamma_flip, ma_trend, vwap_status):
    """根據 GEX 關鍵價位產生戰術建議，避免 None 比較錯誤"""
    advice = []
    if current_price is None:
        return "⚠️ 無 GEX 數據，無法提供戰術建議"
    
    if call_wall is not None:
        dist_to_call = (call_wall - current_price) / current_price * 100
        if dist_to_call < 1 and dist_to_call > 0:
            advice.append(f"🔴 股價距離 Call Wall ${call_wall:.2f} 僅 {dist_to_call:.2f}%，面臨強壓力，不宜追高")
        elif dist_to_call < 0:
            advice.append(f"🚨 股價已突破 Call Wall ${call_wall:.2f}，做市商可能轉為支撐，多頭強勢")
        else:
            advice.append(f"📈 股價低於 Call Wall {dist_to_call:.2f}%，上行空間充足")
    else:
        advice.append("📊 Call Wall 數據暫無")
    
    if put_wall is not None:
        dist_to_put = (current_price - put_wall) / current_price * 100
        if dist_to_put < 1 and dist_to_put > 0:
            advice.append(f"🟢 股價接近 Put Wall ${put_wall:.2f} ({dist_to_put:.2f}%)，下方有強支撐")
        elif dist_to_put < 0:
            advice.append(f"⚠️ 股價跌破 Put Wall ${put_wall:.2f}，空頭主導，加速下跌風險")
        else:
            advice.append(f"📉 股價高於 Put Wall {dist_to_put:.2f}%，支撐距離較遠")
    else:
        advice.append("📊 Put Wall 數據暫無")
    
    if gamma_flip is not None:
        if current_price > gamma_flip:
            advice.append(f"⚡ 站上 Gamma Flip ${gamma_flip:.2f}，做市商轉向撐盤，偏多看待")
        else:
            advice.append(f"⚡ 低於 Gamma Flip ${gamma_flip:.2f}，做市商仍為負 Gamma，易加速下跌")
    else:
        advice.append("📊 Gamma Flip 數據暫無")
    
    if ma_trend == "多頭排列":
        advice.append("🐂 均線多頭排列，中期趨勢強勁，拉回可布局")
    elif ma_trend == "空頭排列":
        advice.append("🐻 均線空頭排列，反彈皆賣點")
    
    if vwap_status != "N/A":
        if "站上" in vwap_status:
            advice.append("📊 站上 VWAP，短線多頭掌控")
        else:
            advice.append("📊 跌破 VWAP，短線偏空")
...
def gex_plot(ticker):
    ...
    try:
        ...
        if gex_result is None or gex_result.get('strikes') is None:
            ...
            <body style="background:#121214;color:#fff;padding:20px;font-family:sans-serif;">
            <h3>❌ {ticker} 無期權數據或目前無法取得</h3>
            <p>👉 建議使用外部專業期權平台查詢：</p>
            <a href="{barchart_url}" target="_blank" style="background:#1565c0;padding:8px 16px;border-radius:20px;color:white;text-decoration:none;">📊 查看 {ticker} Barchart 期權鏈</a>
            <br><br>
            <a href="/indicators/{ticker}">🔙 返回個股頁面</a>
            </body>
            </html>
            """
        ...
        call_wall_str = f"${call_wall:.2f}" if call_wall is not None else "N/A"
        put_wall_str = f"${put_wall:.2f}" if put_wall is not None else "N/A"
        gamma_flip_str = f"${gamma_flip:.2f}" if gamma_flip is not None else "N/A"
        net_gex_str = f"{net_gex:,.0f}"
        total_pos_str = f"{total_pos:,.0f}"
        total_neg_str = f"{total_neg:,.0f}"
        
        same_wall_flip = (call_wall is not None and gamma_flip is not None and abs(call_wall - gamma_flip) < 0.01)
        flip_highlight = "⭐️ 關鍵轉折點" if same_wall_flip else ""
        
        if current_price is not None:
            call_dist_pct = ((call_wall - current_price) / current_price * 100) if call_wall is not None else None
            put_dist_pct = ((put_wall - current_price) / current_price * 100) if put_wall is not None else None
            flip_dist_pct = ((gamma_flip - current_price) / current_price * 100) if gamma_flip is not None else None
        else:
        ...
        fig.add_trace(go.Bar(
            x=strikes, y=gex_values, name='GEX 數值',
            marker_color=bar_colors, marker_line_width=0,
            hovertemplate='履約價: $%{x}<br>GEX: %{y:,.0f}<extra></extra>'
        ))
        
        fig.add_hline(y=0, line_dash="solid", line_color="rgba(255,255,255,0.6)", line_width=3,
                      annotation_text="⚖️ 多空分界", annotation_position="bottom right",
                      annotation_font_size=11, annotation_font_color="#cccccc")
        ...
        if current_price is not None:
            fig.add_vline(x=current_price, line_dash="solid", line_color="#ffffff", line_width=2,
                          ...
                          annotation_font_size=12, annotation_bgcolor="rgba(0,0,0,0.6)")
        if call_wall is not None and call_dist_pct is not None:
            fig.add_vline(x=call_wall, line_dash="dashdot", line_color="#00ff00", line_width=2,
                          annotation_text=f"🧱 Call Wall ${call_wall:.2f}<br>(+{call_dist_pct:.2f}%)",
                          annotation_position="top", annotation_font_size=11, annotation_font_color="#00ff00",
                          annotation_bgcolor="rgba(0,0,0,0.6)")
        if put_wall is not None and put_dist_pct is not None:
            fig.add_vline(x=put_wall, line_dash="dashdot", line_color="#ff6666", line_width=2,
                          annotation_text=f"🛡️ Put Wall ${put_wall:.2f}<br>({put_dist_pct:.2f}%)",
                          annotation_position="bottom", annotation_font_size=11, annotation_font_color="#ff6666",
                          annotation_bgcolor="rgba(0,0,0,0.6)")
        if gamma_flip is not None and flip_dist_pct is not None:
            annotation_text = f"⚡ Gamma Flip ${gamma_flip:.2f}<br>({flip_dist_pct:.2f}%)"
            if same_wall_flip:
                annotation_text = f"🌟 關鍵轉折點 ${gamma_flip:.2f}<br>({flip_dist_pct:.2f}%) (Call Wall)"
            fig.add_vline(x=gamma_flip, line_dash="dot", line_color="#ffaa00", line_width=2.5,
                          ...
                          annotation_font_size=11, annotation_font_color="#ffaa00",
                          annotation_bgcolor="rgba(0,0,0,0.7)", annotation_borderwidth=1,
                          annotation_bordercolor="#ffaa00")
        
        fig.update_layout(
            title=dict(text=f"📊 {ticker} 期權 Gamma 曝險分析", font=dict(size=18, color="#ffffff"), x=0),
            xaxis_title="履約價 (Strike Price)", yaxis_title="Gamma Exposure (GEX) 單位: USD",
            template='plotly_dark', width=1300, height=600, hovermode='x unified',
            legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='center', x=0.5, font=dict(size=12)),
            xaxis=dict(tickangle=-45, tickfont=dict(size=10), nticks=20, gridcolor='rgba(255,255,255,0.08)'),
            yaxis=dict(tickfont=dict(size=10), gridcolor='rgba(255,255,255,0.08)', tickformat=',.0f'),
            plot_bgcolor='#0e0e12', paper_bgcolor='#0e0e12', margin=dict(l=80, r=80, t=100, b=80)
        )
        
        dashboard_html = f"""
        <div style="background:#1e1e1e; border-radius:12px; padding:15px; margin-bottom:20px; display:flex; flex-wrap:wrap; gap:20px; justify-content:space-between; align-items:center;">
            <div style="display:flex; gap:30px; flex-wrap:wrap;">
                <div><strong>📌 當前股價</strong><br>{current_price_str}</div>
                <div><strong>🧱 Call Wall</strong><br>{call_wall_str} {flip_highlight}</div>
                <div><strong>🛡️ Put Wall</strong><br>{put_wall_str}</div>
                <div><strong>⚡ Gamma Flip</strong><br>{gamma_flip_str} {flip_highlight}</div>
                <div><strong>📊 淨 GEX</strong><br>{net_gex_str}</div>
                <div><strong>📈 正 GEX (Call)</strong><br>{total_pos_str}</div>
                <div><strong>📉 負 GEX (Put)</strong><br>{total_neg_str}</div>
            </div>
            <div style="color:#aaa; font-size:12px;">🕒 更新時間: {update_time_str}</div>
        </div>
        """
        ...
        <!DOCTYPE html>
        <html>
        <head><meta charset="UTF-8"><title>{ticker} Gamma Exposure 分析</title>
        <style>body{{background:#121214; color:#fff; font-family:sans-serif; padding:20px; margin:0;}}
        .container{{max-width:1400px; margin:0 auto;}}
        .back-link{{display:inline-block; margin-bottom:20px; background:#2a2a35; padding:6px 14px; border-radius:20px; color:white; text-decoration:none;}}</style>
        </head>
        <body>
        <div class="container">
            <a href="/indicators/{ticker}" class="back-link">🔙 返回個股頁面</a>
            {dashboard_html}
            {plot_html}
        </div>
        </body>
...
def gex_screener():
    ...
    for item in data:
        if "last_update" in item:
            ...
            item["last_update_str"] = "未知"
    return render_template_string(GEX_SCREENER_HTML, stocks=data)

@app.route('/battle_map')
def battle_map():
    """盤前戰術板：列出所有美股關鍵價位與操作建議"""
    try:
        with open(GEX_SCREENER_FILE, 'r', encoding='utf-8') as f:
            gex_items = json.load(f)
            gex_data = {item['ticker']: item for item in gex_items}
    except:
        gex_data = {}
    try:
        with open(AI_SCORES_FILE, 'r', encoding='utf-8') as f:
            ai_items = json.load(f)
            ai_data = {item['ticker']: item for item in ai_items if item.get('market')=='us'}
    except:
        ai_data = {}
    ...
    for ticker in tickers:
        ...
        stocks_info.append({
            ...
            "trend": ai_trend,
            "advice": advice,
            "last_update": gex.get('last_update_str', '未知')
        })
    # 排序：淨 GEX 大的排前面（處理 None）
...
def simulator(ticker):
    """情境模擬器：手動輸入模擬價格，返回 GEX 壓力變化"""
    ticker = ticker.upper()
    try:
        sim_price = float(request.args.get('price', 0))
    except:
        sim_price = 0
    ...
    def calc_analysis(price):
        ...
        if call_wall is not None:
            ...
            data["call_dist_pct"] = None
            data["call_status"] = "N/A"
        if put_wall is not None:
        ...
        if gamma_flip is not None:
            ...
            data["flip_dist_pct"] = None
            data["flip_status"] = "N/A"
        return data
...
def index():
    try:
        with open(AI_SCORES_FILE, 'r', encoding='utf-8') as f: all_scores = json.load(f)
    except: all_scores = []
    tw_stocks = [s for s in all_scores if s.get('market')=='tw'][:10]
    us_stocks = [s for s in all_scores if s.get('market')=='us'][:10]
    try:
        with open(ALERTS_FILE, 'r', encoding='utf-8') as f: alerts = json.load(f)[:10]
    except: alerts = []
    market_temp, market_status = calculate_market_temperature()
    us_sector = get_us_sector_performance()
    tw_sector = get_tw_sector_performance()
    vix = get_vix()
...
def tw_page():
    ...
    try:
        with open(AI_SCORES_FILE, 'r', encoding='utf-8') as f: all_stocks = json.load(f)
...
def tenbagger_radar_page():
    try:
        with open(TENBAGGER_FILE, 'r', encoding='utf-8') as f: candidates = json.load(f)
    except: candidates = []
...
def indicators_page(ticker):
    ...
    if df.empty or curr==0: return render_template_string(INDICATORS_HTML, ticker=ticker, error="無法取得資料")
    close_series = safe_get_close(df)
    ...
    if len(close_series)>=35: _,_,hist,macd_status = calculate_macd(close_series)
    else: macd_status, hist = "-", 0
    vwap = calculate_vwap(df)
    vwap_status = get_vwap_status(curr, vwap) if vwap else "N/A"
    close = df['Close'].values
    # 均線計算
    ma5 = np.mean(close[-5:]) if len(close)>=5 else curr
    ma10 = np.mean(close[-10:]) if len(close)>=10 else curr
    ma20 = np.mean(close[-20:]) if len(close)>=20 else curr
    ma60 = np.mean(close[-60:]) if len(close)>=60 else curr
    ma120 = np.mean(close[-120:]) if len(close)>=120 else curr
    ma240 = np.mean(close[-240:]) if len(close)>=240 else curr
    ...
    def get_ma_trend(ma_name):
        ...
        if ma_name == 'ma5':
            if len(close) >= 6:
                yesterday_ma5 = np.mean(close[-6:-1])
            else:
                yesterday_ma5 = ma5
            diff = ma5 - yesterday_ma5
        elif ma_name == 'ma10':
            if len(close) >= 11:
                yesterday_ma10 = np.mean(close[-11:-1])
            else:
                yesterday_ma10 = ma10
            diff = ma10 - yesterday_ma10
        elif ma_name == 'ma20':
            if len(close) >= 21:
                yesterday_ma20 = np.mean(close[-21:-1])
            else:
                yesterday_ma20 = ma20
            diff = ma20 - yesterday_ma20
        elif ma_name == 'ma60':
            if len(close) >= 61:
                yesterday_ma60 = np.mean(close[-61:-1])
            else:
                yesterday_ma60 = ma60
            diff = ma60 - yesterday_ma60
        elif ma_name == 'ma120':
            if len(close) >= 121:
                yesterday_ma120 = np.mean(close[-121:-1])
            else:
                yesterday_ma120 = ma120
            diff = ma120 - yesterday_ma120
        elif ma_name == 'ma240':
            if len(close) >= 241:
                yesterday_ma240 = np.mean(close[-241:-1])
            else:
                yesterday_ma240 = ma240
            diff = ma240 - yesterday_ma240
        else:
            diff = 0
        if diff > 0:
            return "up", diff
    ...
    ma5_trend, ma5_diff = get_ma_trend('ma5')
    ma10_trend, ma10_diff = get_ma_trend('ma10')
    ma20_trend, ma20_diff = get_ma_trend('ma20')
    ma60_trend, ma60_diff = get_ma_trend('ma60')
    ma120_trend, ma120_diff = get_ma_trend('ma120')
    ma240_trend, ma240_diff = get_ma_trend('ma240')
    ...
    rf_pred_val, xgb_pred_val, lgb_pred_val = None, None, None
    rf_model, rf_scaler = get_rf_model(ticker)
    if rf_model and rf_scaler:
        feats = calculate_features(df)
        if feats:
    ...
    tactical_advice = generate_tactical_advice(curr, gex_call, gex_put, gex_flip, ma_trend, vwap_status)
    ...
    return render_template_string(INDICATORS_HTML, ticker=ticker, price=round(curr,2), change=round(change,2), pct=round(pct,2),
                                  ...
                                  rf_pred=f"{rf_pred_val:+.1f}%" if rf_pred_val is not None else None,
                                  xgb_pred=f"{xgb_pred_val:+.1f}%" if xgb_pred_val is not None else None,
                                  lgb_pred=f"{lgb_pred_val:+.1f}%" if lgb_pred_val is not None else None,
                                  ...
                                  ma5_trend=ma5_trend, ma5_diff=ma5_diff, ma10_trend=ma10_trend, ma10_diff=ma10_diff,
                                  ma20_trend=ma20_trend, ma20_diff=ma20_diff, ma60_trend=ma60_trend, ma60_diff=ma60_diff,
                                  ma120_trend=ma120_trend, ma120_diff=ma120_diff, ma240_trend=ma240_trend, ma240_diff=ma240_diff,
...
# ================== HTML 模板 ==================
INDEX_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>AI量化監控 - 儀表板</title><style>body{background:#121214;color:#fff;font-family:sans-serif;padding:20px;}.card{background:#1e1e1e;padding:15px;border-radius:12px;margin-bottom:20px;}.metric-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:15px;}.metric{background:#2a2a35;padding:10px;border-radius:8px;}table{width:100%;border-collapse:collapse;margin-top:20px;}th,td{padding:10px;text-align:left;border-bottom:1px solid #333;}th{background:#2a2a35;}.positive{color:#ff4444;}.negative{color:#00ff00;}.btn{background:#2a2a35;padding:6px 14px;border-radius:20px;color:white;text-decoration:none;margin:5px;display:inline-block;}.stock-link{color:#ffcc00;text-decoration:none;font-weight:bold;background:#333;padding:2px 6px;border-radius:4px;}.stock-link:hover{color:#fff;background:#555;}.alert{background:#2a1a1a;border-left:4px solid #ff4444;padding:8px;margin:5px 0;}</style></head><body><h1>🤖 AI 量化監控儀表板</h1><div class="card"><a href="/tw" class="btn">📋 全台股</a><a href="/us" class="btn">📋 全美股</a><a href="/ai_ranking" class="btn">🏆 AI 選股排行榜</a><a href="/tenbagger" class="btn">🚀 十倍股雷達</a><a href="/positions" class="btn">📊 持倉中心</a><a href="/bonding" class="btn">📈 均線糾結選股</a><a href="/gex_screener" class="btn">📊 GEX 篩選器</a><a href="/battle_map" class="btn">🗺️ 盤前戰術板</a><a href="/settings" class="btn">⚙️ 設定</a><a href="/retrain_all" class="btn" style="background:#ff9800;">🔄 重新訓練所有模型</a><a href="/clear_model_cache" class="btn" style="background:#ff9800;">🗑️ 清除快取</a><a href="/force_update" class="btn" style="background:#2196f3;">🔄 強制更新</a></div><div class="card"><h2>📊 市場溫度計</h2><div>市場綜合評分: <strong>{{ market_temp }}/100</strong> - {{ market_status }}</div><div>VIX: {{ "%.2f"|format(vix) }} | 美債: {{ "%.2f"|format(treasury) }}% | 美元: {{ "%.2f"|format(dollar) }}</div><div>🇹🇼 台股報價: {{ shioaji_status }}</div></div><div class="card"><h2>📉 崩盤風險監控</h2><div class="metric-grid"><div class="metric"><strong>📊 巴菲特指標</strong><br>{% if buffett_val %}{{ "%.1f"|format(buffett_val) }}%{% else %}{{ buffett_desc }}{% endif %}<br><a href="{{ buffett_url }}" target="_blank" style="color:#6effb0;">詳細</a></div><div class="metric"><strong>😨 VIX</strong><br>{{ "%.2f"|format(vix) }}<br>{% if vix > 30 %}🔴極度恐慌{% elif vix > 20 %}🟡恐慌升溫{% else %}🟢平靜{% endif %}</div><div class="metric"><strong>😱 恐懼貪婪</strong><br>{{ fear_greed_score }}/100 - {{ fear_greed_level }}<br><a href="{{ fear_greed_url }}" target="_blank" style="color:#6effb0;">走勢</a></div><div class="metric"><strong>📉 殖利率曲線</strong><br>{% if yield_spread %}{{ "%.2f"|format(yield_spread) }}%<br>{{ yield_text }}{% else %}N/A{% endif %}</div><div class="metric"><strong>💰 融資餘額</strong><br>{{ margin_balance }}<br>{{ margin_ratio }}<br><a href="{{ margin_url }}" target="_blank" style="color:#6effb0;">查詢</a></div></div></div><div class="card"><h2>🏭 產業輪動</h2>美股: {% for n,r in us_sector_perf %}{{ n }}:{{ r }}% {% endfor %}<br>台股: {% for n,r in tw_sector_perf %}{{ n }}:{{ r }}% {% endfor %}</div><div class="card"><h2>🚨 警報</h2>{% for a in alerts %}<div class="alert">🚨 {{ a.ticker }} - {{ a.message }}</div>{% else %}無{% endfor %}</div><div class="card"><h2>🇹🇼 台股 AI前10</h2><table><thead><tr><th>代號</th><th>價格</th><th>漲跌</th><th>AI分數</th><th>訊號</th></tr></thead><tbody>{% for s in tw_stocks %}<tr><td><a href="/indicators/{{ s.ticker }}" class="stock-link">{{ s.ticker }}</a></td><td>{{ s.price }}</td><td class="{{ 'positive' if s.pct>=0 else 'negative' }}">{{ s.pct }}%</td><td style="color: {{ '#00ff00' if s.ai_score>=70 else '#ffcc00' if s.ai_score>=50 else '#ff4444' }}">{{ s.ai_score }}</td><td>{{ s.signal }}</td></tr>{% endfor %}</tbody></table></div><div class="card"><h2>🇺🇸 美股 AI前10</h2><table><thead><tr><th>代號</th><th>價格</th><th>漲跌</th><th>AI分數</th><th>訊號</th></tr></thead><tbody>{% for s in us_stocks %}<tr><td><a href="/indicators/{{ s.ticker }}" class="stock-link">{{ s.ticker }}</a></td><td>{{ s.price }}</td><td class="{{ 'positive' if s.pct>=0 else 'negative' }}">{{ s.pct }}%</td><td style="color: {{ '#00ff00' if s.ai_score>=70 else '#ffcc00' if s.ai_score>=50 else '#ff4444' }}">{{ s.ai_score }}</td><td>{{ s.signal }}</td></tr>{% endfor %}</tbody></table></div></body></html>'''
TW_US_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{{ title }}</title><style>body{background:#121214;color:#fff;font-family:sans-serif;padding:20px;}.card{background:#1e1e1e;padding:15px;border-radius:12px;margin-bottom:20px;}.index-grid{display:flex;gap:15px;flex-wrap:wrap;margin-bottom:20px;}.index-item{background:#2a2a35;padding:8px 15px;border-radius:8px;}.positive{color:#ff4444;}.negative{color:#00ff00;}table{width:100%;border-collapse:collapse;}th,td{padding:10px;text-align:left;border-bottom:1px solid #333;}th{background:#2a2a35;}.btn{background:#2a2a35;padding:6px 14px;border-radius:20px;color:white;text-decoration:none;margin:5px;display:inline-block;}.stock-link{color:#ffcc00;text-decoration:none;font-weight:bold;background:#333;padding:2px 6px;border-radius:4px;}</style></head><body><h1>{{ title }}</h1><div><a href="/" class="btn">返回儀表板</a> <a href="/ai_ranking" class="btn">🏆 AI排行榜</a></div><div class="card"><div class="index-grid"><div class="index-item">📊 加權: {{ tw_curr }} <span class="{{ 'positive' if tw_pct>=0 else 'negative' }}">{{ tw_change }}點, {{ tw_pct }}%</span></div><div class="index-item">📊 櫃買: {{ otc_curr }} <span class="{{ 'positive' if otc_pct>=0 else 'negative' }}">{{ otc_change }}點, {{ otc_pct }}%</span></div><div class="index-item">📊 道瓊: {{ dow_curr }} <span class="{{ 'positive' if dow_pct>=0 else 'negative' }}">{{ dow_change }}點, {{ dow_pct }}%</span></div><div class="index-item">📊 那斯達克: {{ nas_curr }} <span class="{{ 'positive' if nas_pct>=0 else 'negative' }}">{{ nas_change }}點, {{ nas_pct }}%</span></div><div class="index-item">📊 費半: {{ sox_curr }} <span class="{{ 'positive' if sox_pct>=0 else 'negative' }}">{{ sox_change }}點, {{ sox_pct }}%</span></div></div></div><div class="card"><form method="POST" action="/add_stock_{{ market }}"><input name="ticker" placeholder="股票代號" required><button type="submit" class="btn">➕ 加入自選</button></form></div><div class="card"><table><thead><tr><th>代號</th><th>價格</th><th>漲跌</th><th>類型</th><th>AI分數</th><th>趨勢</th><th>訊號</th><th>操作</th></tr></thead><tbody>{% for s in stocks %}<tr><td><a href="/indicators/{{ s.ticker }}" class="stock-link">{{ s.ticker }}</a></td><td>{{ s.price }}</td><td class="{{ 'positive' if s.pct>=0 else 'negative' }}">{{ s.pct }}%</td><td>{% if market == 'tw' %}{% if shioaji_status == '🟢 即時模式' %}🟢 即時{% else %}🔴 延遲{% endif %}{% else %}🔵 延遲 15分{% endif %}</td><td>{{ s.ai_score }}</td><td>{{ s.trend }}</td><td>{{ s.signal }}</td><td><a href="/delete/{{ market }}/{{ s.ticker }}" class="btn" style="background:#ff4444;">刪除</a></td></tr>{% endfor %}</tbody></table></div></body></html>'''
AI_RANKING_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>AI選股排行榜</title><style>body{background:#121214;color:#fff;font-family:sans-serif;padding:20px;}table{width:100%;border-collapse:collapse;}th,td{padding:10px;text-align:left;border-bottom:1px solid #333;}th{background:#2a2a35;}.btn{background:#2a2a35;padding:6px 14px;border-radius:20px;color:white;text-decoration:none;margin:5px;}.stock-link{color:#ffcc00;text-decoration:none;font-weight:bold;background:#333;padding:2px 6px;border-radius:4px;}</style></head><body><h1>🏆 AI 共振評分排行榜</h1><a href="/" class="btn">返回首頁</a><div class="card"><table><thead><tr><th>排名</th><th>代號</th><th>市場</th><th>價格</th><th>AI分數</th><th>趨勢</th><th>買賣訊號</th></tr></thead><tbody>{% for s in stocks %}<tr><td>{{ loop.index }}</td><td><a href="/indicators/{{ s.ticker }}" class="stock-link">{{ s.ticker }}</a></td><td>{{ '台股' if s.market=='tw' else '美股' }}</td><td>{{ s.price }}</td><td style="color: {{ '#00ff00' if s.ai_score>=70 else '#ffcc00' if s.ai_score>=50 else '#ff4444' }}">{{ s.ai_score }}/100</td><td>{{ s.trend }}</td><td>{{ s.signal }}</td></tr>{% endfor %}</tbody></table></div></body></html>'''
POSITIONS_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>持倉中心</title><style>body{background:#121214;color:#fff;font-family:sans-serif;padding:20px;}.card{background:#1e1e1e;padding:15px;border-radius:12px;margin-bottom:20px;}table{width:100%;border-collapse:collapse;}th,td{padding:8px;text-align:left;border-bottom:1px solid #333;}th{background:#2a2a35;}.positive{color:#00ff00;}.negative{color:#ff4444;}.btn{background:#2a2a35;padding:6px 14px;border-radius:20px;color:white;text-decoration:none;}.stock-link{color:#ffcc00;text-decoration:none;font-weight:bold;background:#333;padding:2px 6px;border-radius:4px;}</style></head><body><h1>📊 持倉管理</h1><a href="/" class="btn">首頁</a><div class="card"><form method="POST" action="/add_position"><input name="pos_ticker" placeholder="代號" required><input name="cost" placeholder="成本" step="0.01" required><input name="shares" placeholder="股數" required><button type="submit" class="btn">新增/更新</button></form></div><div class="card"><table><thead><tr><th>代號</th><th>股數</th><th>成本</th><th>現價</th><th>市值</th><th>損益</th><th>報酬率</th><th>停損價</th><th>操作</th></tr></thead><tbody>{% for p in positions %}<tr><td><a href="/indicators/{{ p.ticker }}" class="stock-link">{{ p.ticker }}</a></td><td>{{ p.shares }}</td><td>{{ p.cost }}</td><td>{{ p.current_price }}</td><td>{{ p.value }}</td><td class="{{ 'positive' if p.profit>=0 else 'negative' }}">{{ p.profit }}</td><td>{{ p.profit_pct }}%</div></td><td>{{ p.stop_loss }}</div></td><td><a href="/clear_position/{{ p.ticker }}" class="btn" style="background:#ff4444;">歸零</a></td></tr>{% endfor %}</tbody></table></div></body></html>'''
INDICATORS_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{{ ticker }} 技術指標</title><style>body{background:#121214;color:#fff;padding:20px;font-family:sans-serif;}.card{background:#1e1e1e;padding:15px;border-radius:12px;margin-bottom:20px;}.metric-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:15px;}.metric{background:#2a2a35;padding:10px;border-radius:8px;}.btn{background:#2a2a35;padding:8px 14px;border-radius:20px;color:white;text-decoration:none;display:inline-block;margin:5px;}.btn-green{background:#2e7d32;}.btn-blue{background:#1565c0;}.btn-orange{background:#e65100;}.btn-red{background:#c62828;}.link-buttons{display:flex;flex-wrap:wrap;gap:10px;margin-top:10px;}.ma-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;}.ma-metric{position:relative;overflow:hidden;transition:transform 0.2s;}.ma-metric:hover{transform:translateY(-2px);background:#353540;}.ma-label{font-size:12px;color:#aaa;margin-bottom:5px;display:flex;justify-content:space-between;}.ma-value{font-size:18px;font-weight:bold;display:flex;align-items:center;gap:6px;margin-bottom:4px;}.trend-up{color:#ff4444;font-size:14px;}.trend-down{color:#00ff00;font-size:14px;}.trend-flat{color:#ffaa00;font-size:12px;}.progress-bar-container{background-color:#333;border-radius:10px;height:6px;margin:6px 0 4px;overflow:hidden;}.progress-bar-fill{height:100%;border-radius:10px;transition:width 0.3s;}.ratio-text{font-size:10px;text-align:right;color:#ccc;}.badge{padding:4px 12px;border-radius:20px;font-size:14px;font-weight:bold;display:inline-flex;align-items:center;gap:6px;}.badge.bull{background-color:rgba(255,68,68,0.2);color:#ff4444;border:1px solid #ff4444;}.badge.bear{background-color:rgba(0,255,0,0.2);color:#00ff00;border:1px solid #00ff00;}.badge.neutral{background-color:rgba(255,170,0,0.2);color:#ffaa00;border:1px solid #ffaa00;}.pulse{display:inline-block;width:10px;height:10px;border-radius:50%;background-color:currentColor;animation:pulse 1.5s infinite;}@keyframes pulse{0%{opacity:0.4;transform:scale(0.8);}70%{opacity:1;transform:scale(1.2);}100%{opacity:0.4;transform:scale(0.8);}}.tooltip-hover{cursor:help;border-bottom:1px dotted #888;}.alert-box{background:#2a2a35;border-left:4px solid #ffaa00;padding:10px;margin-bottom:15px;border-radius:8px;}</style></head><body><h1>📈 {{ ticker }} 技術指標</h1><div><a href="/" class="btn">返回首頁</a> <a href="/backtest/{{ ticker }}" class="btn">📊 回測</a> <a href="/simulator/{{ ticker }}" class="btn btn-orange">🎯 情境模擬器</a></div><div class="link-buttons"><a href="/gex_plot/{{ ticker }}" target="_blank" class="btn btn-green">📊 本地 GEX</a>{% if market == 'us' %}<a href="{{ research_links.MarketWatch }}" target="_blank" class="btn btn-blue">📈 MarketWatch</a><a href="{{ research_links.Barchart }}" target="_blank" class="btn btn-orange">📊 Barchart 期權</a><a href="{{ research_links.Finviz }}" target="_blank" class="btn">🔍 Finviz</a><a href="{{ research_links.Reddit }}" target="_blank" class="btn btn-red">💬 Reddit</a>{% else %}<a href="{{ research_links.WantGoo }}" target="_blank" class="btn btn-blue">🇹🇼 玩股網</a><a href="{{ research_links.GoodInfo }}" target="_blank" class="btn btn-orange">📊 GoodInfo</a><a href="{{ research_links.Google }}" target="_blank" class="btn">🔍 Google 新聞</a>{% endif %}<a href="{{ research_links.TradingView }}" target="_blank" class="btn btn-blue">📈 TradingView</a><a href="{{ research_links.Yahoo }}" target="_blank" class="btn">📰 Yahoo 財經</a></div>{% if error %}<div class="card">{{ error }}</div>{% else %}<div class="card"><h3>即時報價</h3><div class="metric-grid"><div class="metric">價格: {{ price }}</div><div class="metric">漲跌: {{ change }} ({{ pct }}%)</div><div class="metric">AI評分: {{ ai_score }}/100</div><div class="metric">報價類型: {% if market == 'tw' %}{% if shioaji_status == '🟢 即時模式' %}🟢 即時 (Shioaji){% else %}🔴 延遲 (Yahoo){% endif %}{% else %}🔵 延遲 15分鐘 (Yahoo){% endif %}</div></div></div><div class="card"><h3>⚔️ 戰術建議</h3><div class="alert-box">{{ tactical_advice }}</div></div><div class="card"><h3>核心指標</h3><div class="metric-grid"><div class="metric">RSI: {{ rsi }} {{ rsi_status }}</div><div class="metric">MACD: {{ macd_status }}</div><div class="metric">VWAP: {{ vwap_status }}</div><div class="metric">量比: {{ volume_ratio }}</div></div></div><div class="card"><h3>🤖 AI預測</h3><div class="metric-grid"><div class="metric"><strong>RandomForest</strong><br>預期漲跌: {{ rf_pred if rf_pred else 'N/A' }}</div><div class="metric"><strong>XGBoost</strong><br>預期漲跌: {{ xgb_pred if xgb_pred else 'N/A' }}</div><div class="metric"><strong>LightGBM</strong><br>預期漲跌: {{ lgb_pred if lgb_pred else 'N/A' }}</div><div class="metric"><strong>綜合評分</strong><br>{% if ensemble_score %}{{ ensemble_score }}/100<br><strong>{{ ensemble_signal }}</strong>{% else %}訓練中...{% endif %}</div></div></div><div class="card"><h3>📊 因子</h3><div class="metric-grid"><div class="metric">💰 籌碼: {{ smart_score }}</div><div class="metric">📈 趨勢: {{ trend_score }}</div><div class="metric">📈 成長: {{ growth_score }}</div><div class="metric">🚀 十倍股: {{ ten_bagger_score }}</div></div></div><div class="card"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;"><h3 style="margin:0;" class="tooltip-hover" title="均線趨勢與乖離率分析。紅色箭頭=均線上漲；進度條紅色=股價站上均線。">📈 均線分析</h3><div class="badge {% if ma_trend == '多頭排列' %}bull{% elif ma_trend == '空頭排列' %}bear{% else %}neutral{% endif %}" title="{% if ma_trend == '多頭排列' %}短期均線 > 中期 > 長期，技術強勢{% elif ma_trend == '空頭排列' %}長期均線壓制，技術弱勢{% else %}均線交錯，方向不明{% endif %}"><span class="pulse"></span>{% if ma_trend == '多頭排列' %}🐂 多頭排列{% elif ma_trend == '空頭排列' %}🐻 空頭排列{% else %}⚖️ 混亂{% endif %}</div></div><div class="ma-grid"><div class="metric ma-metric" title="5日均線，乖離率：股價偏離5MA的百分比。正值=站上"><div class="ma-label">5MA <span class="tooltip-hover" title="股價與5MA的距離">乖離</span></div><div class="ma-value">{{ "%.2f"|format(ma5) }}{% if ma5_trend == 'up' %}<span class="trend-up">▲</span>{% elif ma5_trend == 'down' %}<span class="trend-down">▼</span>{% else %}<span class="trend-flat">●</span>{% endif %}</div><div class="progress-bar-container"><div class="progress-bar-fill" style="width:{{ 50+ma5_ratio }}%;background-color:{% if ma5_ratio>0 %}#ff4444{% else %}#00ff00{% endif %};"></div></div><div class="ratio-text">{{ "%.1f"|format(ma5_ratio) }}%</div></div><div class="metric ma-metric" title="10日均線"><div class="ma-label">10MA</div><div class="ma-value">{{ "%.2f"|format(ma10) }}{% if ma10_trend == 'up' %}<span class="trend-up">▲</span>{% elif ma10_trend == 'down' %}<span class="trend-down">▼</span>{% else %}<span class="trend-flat">●</span>{% endif %}</div><div class="progress-bar-container"><div class="progress-bar-fill" style="width:{{ 50+ma10_ratio }}%;background-color:{% if ma10_ratio>0 %}#ff4444{% else %}#00ff00{% endif %};"></div></div><div class="ratio-text">{{ "%.1f"|format(ma10_ratio) }}%</div></div><div class="metric ma-metric" title="20日均線（月線）"><div class="ma-label">20MA</div><div class="ma-value">{{ "%.2f"|format(ma20) }}{% if ma20_trend == 'up' %}<span class="trend-up">▲</span>{% elif ma20_trend == 'down' %}<span class="trend-down">▼</span>{% else %}<span class="trend-flat">●</span>{% endif %}</div><div class="progress-bar-container"><div class="progress-bar-fill" style="width:{{ 50+ma20_ratio }}%;background-color:{% if ma20_ratio>0 %}#ff4444{% else %}#00ff00{% endif %};"></div></div><div class="ratio-text">{{ "%.1f"|format(ma20_ratio) }}%</div></div><div class="metric ma-metric" title="60日均線（季線）"><div class="ma-label">60MA</div><div class="ma-value">{{ "%.2f"|format(ma60) }}{% if ma60_trend == 'up' %}<span class="trend-up">▲</span>{% elif ma60_trend == 'down' %}<span class="trend-down">▼</span>{% else %}<span class="trend-flat">●</span>{% endif %}</div><div class="progress-bar-container"><div class="progress-bar-fill" style="width:{{ 50+ma60_ratio }}%;background-color:{% if ma60_ratio>0 %}#ff4444{% else %}#00ff00{% endif %};"></div></div><div class="ratio-text">{{ "%.1f"|format(ma60_ratio) }}%</div></div><div class="metric ma-metric" title="120日均線（半年線）"><div class="ma-label">120MA</div><div class="ma-value">{{ "%.2f"|format(ma120) }}{% if ma120_trend == 'up' %}<span class="trend-up">▲</span>{% elif ma120_trend == 'down' %}<span class="trend-down">▼</span>{% else %}<span class="trend-flat">●</span>{% endif %}</div><div class="progress-bar-container"><div class="progress-bar-fill" style="width:{{ 50+ma120_ratio }}%;background-color:{% if ma120_ratio>0 %}#ff4444{% else %}#00ff00{% endif %};"></div></div><div class="ratio-text">{{ "%.1f"|format(ma120_ratio) }}%</div></div><div class="metric ma-metric" title="240日均線（年線）"><div class="ma-label">240MA</div><div class="ma-value">{{ "%.2f"|format(ma240) }}{% if ma240_trend == 'up' %}<span class="trend-up">▲</span>{% elif ma240_trend == 'down' %}<span class="trend-down">▼</span>{% else %}<span class="trend-flat">●</span>{% endif %}</div><div class="progress-bar-container"><div class="progress-bar-fill" style="width:{{ 50+ma240_ratio }}%;background-color:{% if ma240_ratio>0 %}#ff4444{% else %}#00ff00{% endif %};"></div></div><div class="ratio-text">{{ "%.1f"|format(ma240_ratio) }}%</div></div></div></div><div class="card"><h3>壓力支撐</h3><div class="metric-grid"><div class="metric">🔴 壓力: {{ resistance }}</div><div class="metric">🟢 目標: {{ target }}</div><div class="metric">⚪ 停損: {{ stop_loss }}</div></div></div><div class="card"><h3>📌 價格警戒線 (GEX)</h3><div class="metric-grid"><div class="metric">🧱 Call Wall: {{ gex_call if gex_call else 'N/A' }}</div><div class="metric">🛡️ Put Wall: {{ gex_put if gex_put else 'N/A' }}</div><div class="metric">⚡ Gamma Flip: {{ gex_flip if gex_flip else 'N/A' }}</div></div></div>{% if options %}<div class="card"><h3>📊 期權信號</h3><div class="metric-grid">{% if options.pcr %}<div class="metric">PCR: {{ options.pcr.pcr }} {{ options.pcr.text }}</div>{% endif %}{% if options.max_pain %}<div class="metric">Max Pain: {{ options.max_pain.max_pain }}<br>{{ options.max_pain.signal }}</div>{% endif %}{% if options.iv %}<div class="metric">IV Rank: {{ options.iv.iv }}%<br>{{ options.iv.text }}</div>{% endif %}{% if options.gex %}<div class="metric">Call Wall: {{ options.gex.call_wall }}<br>Put Wall: {{ options.gex.put_wall }}<br>Gamma Flip: {{ options.gex.gamma_flip_strike }}</div>{% endif %}</div><div class="metric" style="text-align:center;"><strong style="color:{{ options.composite.color }};">{{ options.composite.text }}</strong><br>{{ options.composite.reasons|join(' ｜ ') }}</div></div>{% endif %}<div class="card"><h3>📖 評分解釋</h3><ul>{% for r in reasons %}<li>{{ r }}</li>{% endfor %}</ul></div><div class="card"><h3>📊 回測績效</h3><div class="metric-grid" id="backtest_stats"><div class="metric">載入中...</div></div></div>{% endif %}<script>fetch('/backtest_stats/{{ ticker }}').then(r=>r.json()).then(d=>{if(d.error)document.getElementById('backtest_stats').innerHTML='<div class="metric">無資料</div>';else document.getElementById('backtest_stats').innerHTML=`<div class="metric">總訊號: ${d.total_signals}次</div><div class="metric">勝率: ${d.win_rate}%</div><div class="metric">平均獲利: +${d.avg_profit}%</div><div class="metric">平均虧損: ${d.avg_loss}%</div><div class="metric">Profit Factor: ${d.profit_factor}</div>`;})</script></body></html>'''
...
TENBAGGER_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>十倍股雷達</title><style>body{background:#121214;color:#fff;font-family:sans-serif;padding:20px;}table{width:100%;border-collapse:collapse;}th,td{padding:8px;text-align:left;border-bottom:1px solid #333;}th{background:#2a2a35;}.score-high{color:#00ff00;font-weight:bold;}.btn{background:#2a2a35;padding:6px 14px;border-radius:20px;color:white;text-decoration:none;margin:5px;}.stock-link{color:#ffcc00;text-decoration:none;font-weight:bold;background:#333;padding:2px 6px;border-radius:4px;}</style></head><body><h1>🚀 十倍股候選雷達</h1><a href="/" class="btn">首頁</a><div class="card"><p>⚡ 條件：營收成長>30% / 毛利率成長 / 52週新高 / 爆量 / 空頭比例</p></div><div class="card"></td><thead><tr><th>排名</th><th>代號</th><th>價格</th><th>分數</th><th>條件</th></tr></thead><tbody>{% for c in candidates %}<tr><td>{{ loop.index }}</td><td><a href="/indicators/{{ c.ticker }}" class="stock-link">{{ c.ticker }}</a></td><td>{{ c.price }}</td><td class="score-high">{{ c.score }}/6</div></td><td>{{ c.conditions|join(', ') }}</div></td></tr>{% endfor %}</tbody></table></div></body></html>'''
BONDING_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>均線糾結選股</title><style>body{background:#121214;color:#fff;font-family:sans-serif;padding:20px;}.card{background:#1e1e1e;padding:15px;border-radius:12px;margin-bottom:20px;}table{width:100%;border-collapse:collapse;}th,td{border:1px solid #444;padding:8px;text-align:left;}th{background:#2a2a35;}.stock-link{color:#ffcc00;text-decoration:none;font-weight:bold;background:#333;padding:2px 6px;border-radius:4px;}.btn{background:#2a2a35;padding:6px 14px;border-radius:20px;color:white;text-decoration:none;}</style></head><body><h1>📈 均線糾結選股</h1><div class="note">📊 糾結閾值: {{ threshold }}%</div><div class="card"><h2>🇹🇼 台股 ({{ tw_count }})</h2>{% if tw_results %}</td><thead><tr><th>代號</th><th>價格</th><th>糾結度%</th><th>MA5</th><th>MA20</th><th>MA60</th><th>方向</th></tr></thead><tbody>{% for s in tw_results %}<tr><td><a href="/indicators/{{ s.ticker }}" class="stock-link">{{ s.ticker }}</a></td><td>{{ s.price }}</td><td>{{ s.spread }}%</td><td>{{ s.ma5 }}</td><td>{{ s.ma20 }}</td><td>{{ s.ma60 }}</td><td>{{ s.direction }}{% if s.volume_confirm %}📊{% endif %}</td></tr>{% endfor %}</tbody></table>{% else %}<p>無</p>{% endif %}</div><div class="card"><h2>🇺🇸 美股 ({{ us_count }})</h2>{% if us_results %}<table><thead><tr><th>代號</th><th>價格</th><th>糾結度%</th><th>MA5</th><th>MA20</th><th>MA60</th><th>方向</th></tr></thead><tbody>{% for s in us_results %}<tr><td><a href="/indicators/{{ s.ticker }}" class="stock-link">{{ s.ticker }}</a></td><td>{{ s.price }}</td><td>{{ s.spread }}%</td><td>{{ s.ma5 }}</td><td>{{ s.ma20 }}</td><td>{{ s.ma60 }}</td><td>{{ s.direction }}{% if s.volume_confirm %}📊{% endif %}</td></tr>{% endfor %}</tbody></table>{% else %}<p>無</p>{% endif %}</div><a href="/" class="btn">返回首頁</a></body></html>'''
SETTINGS_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>系統設定</title><style>body{background:#121214;color:#fff;padding:20px;font-family:sans-serif;}.card{background:#1e1e1e;padding:15px;border-radius:12px;margin-bottom:20px;}input,button{padding:8px;margin:5px;border-radius:20px;border:none;}button{background:#4caf50;color:white;}.btn{background:#2a2a35;padding:6px 14px;border-radius:20px;color:white;text-decoration:none;display:inline-block;}</style></head><body><h1>⚙️ 策略設定</h1><div><a href="/" class="btn">返回首頁</a></div><form method="POST"><div class="card"><h3>風險控制</h3><label>單筆最大虧損(%): <input type="number" step="0.5" name="max_loss_per_trade" value="{{ settings.max_loss_per_trade }}"></label><br><label>單一持股最大佔比(%): <input type="number" step="5" name="max_concentration" value="{{ settings.max_concentration }}"></label><br><label>現金餘額: <input type="number" step="1000" name="cash_balance" value="{{ settings.cash_balance }}"></label><br><label>均線糾結閾值(%): <input type="number" step="0.5" name="bonding_threshold" value="{{ settings.bonding_threshold }}"></label><br></div><div class="card"><h3>系統效能</h3><label>背景更新間隔(分鐘): <input type="number" name="background_update_minutes" value="{{ settings.background_update_minutes }}"></label><br><label>資料快取時間(分鐘): <input type="number" name="cache_ttl_minutes" value="{{ settings.cache_ttl_minutes }}"></label><br><label>觀察名單: <input type="text" name="watchlist" value="{{ watchlist }}" style="width:80%;"></label><br></div><button type="submit">儲存設定</button></form></body></html>'''
GEX_SCREENER_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>GEX 篩選器 - 美股淨 Gamma Exposure</title><style>body{background:#121214;color:#fff;font-family:sans-serif;padding:20px;}.card{background:#1e1e1e;padding:15px;border-radius:12px;margin-bottom:20px;}table{width:100%;border-collapse:collapse;}th,td{padding:10px;text-align:left;border-bottom:1px solid #333;}th{background:#2a2a35;}.positive{color:#00ff00;}.negative{color:#ff4444;}.btn{background:#2a2a35;padding:6px 14px;border-radius:20px;color:white;text-decoration:none;margin:5px;display:inline-block;}.stock-link{color:#ffcc00;text-decoration:none;font-weight:bold;background:#333;padding:2px 6px;border-radius:4px;}</style></head><body><h1>📊 美股 Gamma Exposure (GEX) 篩選器</h1><div><a href="/" class="btn">返回儀表板</a> <a href="/force_update_gex" class="btn" style="background:#2196f3;">🔄 手動更新</a></div><div class="card"><p>⭐ 淨 GEX = 正 GEX (Call) + 負 GEX (Put)，數值越大代表做市商越可能在該價位提供流動性。</p></div><div class="card"><table><thead><tr><th>代號</th><th>現價</th><th>淨 GEX</th><th>正 GEX (Call)</th><th>負 GEX (Put)</th><th>Call Wall</th><th>Put Wall</th><th>Gamma Flip</th><th>更新時間</th></tr></thead><tbody>{% for s in stocks %}<tr><td><a href="/indicators/{{ s.ticker }}" class="stock-link">{{ s.ticker }}</a></td><td>{{ "${:.2f}".format(s.current_price) if s.current_price else "N/A" }}</td><td class="{{ 'positive' if s.net_gex and s.net_gex > 0 else 'negative' if s.net_gex and s.net_gex < 0 else '' }}">{{ "{:,.0f}".format(s.net_gex) if s.net_gex is not none else "N/A" }}</div></td><td>{{ "{:,.0f}".format(s.positive_gex) if s.positive_gex is not none else "N/A" }}</div></td><td>{{ "{:,.0f}".format(s.negative_gex) if s.negative_gex is not none else "N/A" }}</div></td><td>{{ "${:.2f}".format(s.call_wall) if s.call_wall else "N/A" }}</div></td><td>{{ "${:.2f}".format(s.put_wall) if s.put_wall else "N/A" }}</div></td><td>{{ "${:.2f}".format(s.gamma_flip) if s.gamma_flip else "N/A" }}</div></div></td><td>{{ s.last_update_str }}</div></tr>{% endfor %}</tbody></table></div></body></html>'''
...
<thead>
  <tr>
    <th>代號</th><th>現價</th><th>Call Wall</th><th>Put Wall</th><th>Gamma Flip</th><th>淨 GEX</th><th>均線趨勢</th><th>AI分數</th><th>戰術建議</th>
   </tr>
</thead>
<tbody>
{% for s in stocks %}
  <tr>
    ...
    <td>{{ "${:.2f}".format(s.current_price) if s.current_price else "N/A" }}</div></td>
    <td>{{ "${:.2f}".format(s.call_wall) if s.call_wall else "N/A" }}</div></td>
    <td>{{ "${:.2f}".format(s.put_wall) if s.put_wall else "N/A" }}</div></td>
    <td>{{ "${:.2f}".format(s.gamma_flip) if s.gamma_flip else "N/A" }}</div></td>
    <td class="{{ 'positive' if s.net_gex and s.net_gex > 0 else 'negative' if s.net_gex and s.net_gex < 0 else '' }}">{{ "{:,.0f}".format(s.net_gex) if s.net_gex is not none else "N/A" }}</div></td>
    <td>{{ s.trend }}</div></td>
    <td>{{ s.ai_score }}</div></td>
    <td class="advice">{{ s.advice }}</div></td>
   </tr>
{% endfor %}
</tbody>
</table>

...
if not os.path.exists(POSITIONS_FILE): json.dump(
    [], open(POSITIONS_FILE, "w", encoding='utf-8'))
if not os.path.exists(WATCHLIST_FILE): json.dump(
...
def get_growth_score(ticker, df):
    ...
    try:
        ...
        if any(k in sector for k in [
               "AI", "SEMICONDUCTOR", "CLOUD", "NUCLEAR", "ROBOTICS"]): score += 20
    except: pass
    return min(100, score), []
...
def get_ten_bagger_score(ticker):
    ...
    try:
        ...
        if any(x in ticker for x in [
               "NVDA", "AMD", "AVGO", "MRVL", "VRT", "SMCI", "PLTR", "OKLO", "IONQ"]): score += 30
    except: pass
    return min(100, score)
...
def get_options_signals(ticker):
    ...
    if mp:
        if mp['dist_pct'] < - \
            5: score += 1.5; reasons.append(f"低於 Max Pain {abs(mp['dist_pct']):.1f}%")
...
def safe_get_close(df):
    ...
    try:
        if isinstance(
            df.columns, pd.MultiIndex): df.columns = df.columns.get_level_values(0)
        close = df['Close']
...
def get_us_stock_data(ticker):
    ...
    try:
        ...
        curr = fast.get('last_price', None); prev = fast.get(
            'previous_close', None)
...
def compute_stock_summary(ticker, market):
    try:
        ...
        if len(close_series) >= 35: _, _, _, macd_status = calculate_macd(
            close_series)
        else: macd_status = "-"
        vwap = calculate_vwap(df)
...
def tenbagger_radar(ticker, market):
    try:
            ...
            if not inc.empty and 'Gross Profit' in inc.index and 'Total Revenue' in inc.index:
                ...
                if len(gp) >= 2 and len(rev) >= 2:
                    gm1 = gp.iloc[0]/rev.iloc[0] * \
                        100; gm2 = gp.iloc[1]/rev.iloc[1]*100
                    gm_growth = gm1 - gm2
        except: pass
        ...
        score = 0; cond = []
        if revenue_growth > 30: score += 1; cond.append(
...
def scan_alerts():
    ...
    try:
        with open(AI_SCORES_FILE, 'r',
                  encoding='utf-8') as f: stocks = json.load(f)
...
def calculate_market_temperature():
    vix = get_vix(); yield_val = get_treasury_yield(
    ); dollar = get_dollar_index(); trend = get_market_trend()
...
def generate_tactical_advice(current_price, call_wall, put_wall, gamma_flip, ma_trend, vwap_status, ai_signal=None, unusual_options=None):
    ...
    if call_wall is not None:
        ...
        if dist < 1 and dist > 0: advice.append(
    ...
    if gamma_flip is not None:
        if current_price > gamma_flip: advice.append(
...
def gex_screener():
    try:
        with open(GEX_SCREENER_FILE, 'r',
                  encoding='utf-8') as f: data = json.load(f)
    except: data = []
    for item in data:
...
def battle_map():
    ...
    for ticker in get_all_tickers('us'):
        ...
        call_wall = gex.get('call_wall'); put_wall = gex.get(
            'put_wall'); gamma_flip = gex.get('gamma_flip')
...
def simulator(ticker):
    ...
    current_price = gex_result.get('current_price')
    call_wall = gex_result.get('call_wall'); put_wall = gex_result.get(
        'put_wall'); gamma_flip = gex_result.get('gamma_flip_strike')
...
def index():
    ...
    vix = get_vix(); treasury = get_treasury_yield(
    ); dollar = get_dollar_index(); shioaji_status = get_shioaji_status()

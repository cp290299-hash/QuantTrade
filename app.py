...
if not os.path.exists(POSITIONS_FILE):
    json.dump(
        [], open(POSITIONS_FILE, "w", encoding='utf-8'))
if not os.path.exists(WATCHLIST_FILE):
    json.dump(
        ["NVDA", "TSLA", "AAPL", "MSFT", "GOOGL"], open(WATCHLIST_FILE, "w", encoding='utf-8'))
...
def get_tw_stock_realtime_shioaji(ticker, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.debug(
                f"Shioaji 抓取 {ticker} 失敗 (嘗試 {attempt+1}/{max_retries}): {e}")
            time.sleep(0.1 * (attempt + 1))
            continue
...
def calculate_features(df):
    ...
    high = df['High'].values
    low = df['Low'].values
    volume = df['Volume'].values if 'Volume' in df.columns else np.ones(
        len(close))
    features = {}
    features['return_1d'] = (close[-1]-close[-2]) / \
        close[-2] if len(close) >= 2 else 0
    features['return_5d'] = (close[-1]-close[-6]) / \
        close[-6] if len(close) >= 6 else 0
    features['return_10d'] = (close[-1]-close[-11]) / \
        close[-11] if len(close) >= 11 else 0
    features['return_20d'] = (close[-1]-close[-21]) / \
        close[-21] if len(close) >= 21 else 0
    ...
    features['bb_position'] = (close[-1]-bb_lower) / \
        (bb_upper-bb_lower) if bb_upper != bb_lower else 0.5
    ...
    features['high_low_ratio'] = (
        high[-1]-low[-1])/close[-1] if close[-1] > 0 else 0
    features['high_20d_ratio'] = (
        high[-1]-np.max(high[-20:]))/close[-1] if len(high) >= 20 and close[-1] > 0 else 0
    features['low_20d_ratio'] = (
        low[-1]-np.min(low[-20:]))/close[-1] if len(low) >= 20 and close[-1] > 0 else 0
    features['trend_strength'] = abs(
        close[-1]-close[-20])/ma20 if ma20 > 0 else 0
    return features
...
def prepare_training_data(ticker, days=180):
    try:
        ...
        for i in range(60, len(df)-settings.get('prediction_days', 3)):
            ...
            if features is None:
                continue
            future_close = df['Close'].iloc[i +
                                            settings.get('prediction_days', 3)]
...
def get_trend_score(df):
    ...
    price = close.iloc[-1]
    rsi = calculate_rsi(
        close).iloc[-1] if not calculate_rsi(close).isna().all() else 50
...
def get_growth_score(ticker, df):
    ...
    try:
        ...
        if any(k in sector for k in [
               "AI", "SEMICONDUCTOR", "CLOUD", "NUCLEAR", "ROBOTICS"]):
            score += 20
    except:
        pass
    return min(100, score), []
...
def get_ten_bagger_score(ticker):
    ...
    try:
        info = yf.Ticker(ticker).info
        text = (ticker + " " + info.get('sector', '') + " " + info.get('industry',
                '') + " " + info.get('longBusinessSummary', '')).upper()
        keywords = ["AI", "ROBOT", "NUCLEAR", "QUANTUM", "CLOUD", "BIOTECH"]
        for kw in keywords:
            if kw in text:
                score += 20
        if any(x in ticker for x in [
               "NVDA", "AMD", "AVGO", "MRVL", "VRT", "SMCI", "PLTR", "OKLO", "IONQ"]):
            score += 30
    except:
        pass
    return min(100, score)
...
def ensemble_predict(ticker, df):
    ...
    final = sum(scores[m] * weights[m]
                for m in scores if m in weights) / total_w
    final = round(final, 1)
...
def calculate_put_call_ratio(ticker):
    ...
    if pcr < 0.7:
        sig, txt = "bullish", "☀️ 偏多 (Call 活跃)"
    elif pcr > 1.3:
        sig, txt = "bearish", "🌧️ 偏空 (Put 活跃)"
    else:
        sig, txt = "neutral", "⛅ 中性"
...
def calculate_max_pain(ticker):
    ...
    for strike in strikes:
        call_pain = sum((row['strike']-strike)*row['openInterest']
                        for _, row in calls.iterrows() if row['strike'] > strike)
        put_pain = sum((strike-row['strike'])*row['openInterest']
    ...
    if dist_pct > 5:
        signal = "⚠️ 高於 Max Pain 5%+ → 可能下跌"
    elif dist_pct < -5:
        signal = "📈 低於 Max Pain 5%+ → 可能上漲"
    else:
        signal = "📍 接近 Max Pain → 價格有吸引力"
...
def calculate_iv_rank(ticker):
    ...
    iv = (call_atm['impliedVolatility'].mean() +
          put_atm['impliedVolatility'].mean()) / 2 * 100
    iv = round(iv, 2) if not np.isnan(iv) else 30
    if iv < 30:
        interp = "❄️ IV 偏低 (選擇權便宜)"
    elif iv > 70:
        interp = "🔥 IV 偏高 (選擇權貴)"
    else:
        interp = "⛅ IV 中性"
...
def calculate_gamma_exposure(ticker, current_price=None, r=0.05):
    ...
    try:
        ...
        historical_vol = hist_data['Close'].pct_change().dropna(
        ).std() * np.sqrt(252) if not hist_data.empty else 0.3
        gex_data = {}
        for exp in exps[:3]:
            try:
                ...
                if opt.calls.empty and opt.puts.empty:
                    continue
                T = max((datetime.strptime(exp, '%Y-%m-%d') -
                        datetime.now()).days / 365, 1/365)
                for _, row in opt.calls.iterrows():
                    strike = row['strike']
                    if abs(strike - current_price) / current_price > 0.3:
                        continue
                    oi = row['openInterest'] if not pd.isna(
                        row['openInterest']) else 0
                    iv = row['impliedVolatility'] if not pd.isna(
                        row['impliedVolatility']) else historical_vol
                    if oi <= 0:
                        continue
                    gamma = black_scholes_gamma(
                        current_price, strike, T, r, iv if iv > 0 else historical_vol)
        ...
        for i in range(1, len(gex_values)):
            if gex_values[i-1] < 0 and gex_values[i] > 0:
                ...
                break
        pos_gex = [(strikes[i], gex_values[i])
                   for i in range(len(strikes)) if gex_values[i] > 0]
        neg_gex = [(strikes[i], gex_values[i])
                   for i in range(len(strikes)) if gex_values[i] < 0]
...
def get_options_signals(ticker):
    ...
    if pcr:
        if pcr['signal'] == 'bullish':
            score += 2
            reasons.append("PCR 偏多")
        elif pcr['signal'] == 'bearish':
            score -= 2
            reasons.append("PCR 偏空")
    if mp:
        if mp['dist_pct'] < - \
                5:
            score += 1.5
            reasons.append(f"低於 Max Pain {abs(mp['dist_pct']):.1f}%")
        elif mp['dist_pct'] > 5:
            score -= 1.5
            reasons.append(f"高於 Max Pain {mp['dist_pct']:.1f}%")
    if iv:
        if iv['iv'] < 30:
            score += 1
            reasons.append("IV 偏低")
        elif iv['iv'] > 70:
            score -= 1
    ...
    if gex:
        if gex['call_wall'] and gex['current_price'] > gex['call_wall']:
            score -= 1.5
            reasons.append(
                f"高於 Call Wall ${gex['call_wall']:.2f}")
        if gex['put_wall'] and gex['current_price'] < gex['put_wall']:
            score += 1.5
            reasons.append(f"低於 Put Wall ${gex['put_wall']:.2f}")
        if gex['gamma_flip_strike'] and gex['current_price'] > gex['gamma_flip_strike']:
            score += 1
            reasons.append(
                f"站上 Gamma Flip ${gex['gamma_flip_strike']:.2f}")
    if score >= 2:
        composite, color = "🔥 期權信號強烈偏多", "#00ff00"
    elif score >= 0.5:
        composite, color = "📈 期權信號偏多", "#88ff88"
    elif score <= -2:
        composite, color = "❄️ 期權信號強烈偏空", "#ff4444"
    elif score <= -0.5:
        composite, color = "📉 期權信號偏空", "#ff8844"
    else:
        composite, color = "⛅ 期權信號中性", "#cccccc"
...
def get_all_tickers(market):
    with get_db_connection() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT ticker FROM user_watchlist WHERE market = %s", (market,))
        rows = cur.fetchall()
...
def compute_stock_summary(ticker, market):
    try:
        ...
        if len(close_series) >= 35:
            _, _, _, macd_status = calculate_macd(
                close_series)
        else:
            macd_status = "-"
        vwap = calculate_vwap(df)
...
def tenbagger_radar(ticker, market):
    try:
        ...
        ).iloc[-1] if len(df) >= 252 else df['High'].max()
        ...
        if market == 'us':
            try:
                short = yf.Ticker(ticker).info.get(
                    'shortPercentOfFloat', 0)*100
                short_high = short > 15
            except:
                pass
        score = 0
        ...
        if revenue_growth > 30:
            score += 1
            cond.append(
                f"營收成長 {revenue_growth:.1f}%")
        if gm_growth > 5:
            score += 1
            cond.append(f"毛利率成長 {gm_growth:.1f}%")
        if is_high:
            score += 1
            cond.append("52週新高")
        if vol_surge:
...
def scan_alerts():
    ...
    for s in stocks[:100]:
        ...
        if vol_ratio > 3:
            alerts.append(
                {"ticker": ticker, "type": "volume_surge", "message": f"成交量爆增 {vol_ratio:.1f}倍", "importance": "high"})
        high_52w = df['High'].rolling(252).max(
        ).iloc[-1] if len(df) >= 252 else df['High'].max()
        if curr >= high_52w*0.99:
            alerts.append(
                {"ticker": ticker, "type": "new_high", "message": "突破52週新高", "importance": "high"})
...
def run_backtest_advanced(ticker, days=180, benchmark_ticker='^GSPC'):
    try:
        ...
        for i in range(20, len(df_test)):
            ...
            price = close[-1]
            deltas = np.diff(
                close[-15:]) if len(close) >= 15 else np.diff(close)
            gain = np.mean(deltas[deltas > 0]) if any(deltas > 0) else 0
            loss = -np.mean(deltas[deltas < 0]) if any(deltas < 0) else 0
        ...
        rf = 0.02/252
        excess = rets - rf
        sharpe = np.sqrt(252)*excess.mean() / \
            rets.std() if rets.std() != 0 else 0
        downside = rets[rets < 0]
        sortino = np.sqrt(
            252)*excess.mean()/downside.std() if len(downside) > 0 and downside.std() != 0 else 0
...
def get_backtest_performance(ticker):
    try:
        ...
        avg_profit = np.mean([r for r in trades if r > 0]) if any(
            r > 0 for r in trades) else 0
        avg_loss = abs(np.mean([r for r in trades if r < 0])) if any(
            r < 0 for r in trades) else 0
...
def get_eod_stock_data(ticker, period="1y"):
    try:
        ...
        params = {'api_token': EOD_API_KEY, 'from': start_date,
        ...
        if response.status_code == 200:
            ...
            df.rename(columns={'adjusted_close': 'Close', 'open': 'Open',
...
def get_treasury_yield():
    try:
        return yf.Ticker("^TNX").history(period="5d")['Close'].iloc[-1]
    except:
        return 4.2
...
def get_dollar_index():
    try:
        return yf.Ticker("DX-Y.NYB").history(period="5d")['Close'].iloc[-1]
    except:
        return 104.0
...
US_SECTOR_ETFS = {"AI": "AIQ", "電力": "XLU",
                  "機器人": "ARKQ", "核能": "URA", "網路安全": "CIBR"}
...
def save_positions(positions):
    with get_db_connection() as conn:
        ...
        for p in positions:
            cur.execute("INSERT INTO user_positions (ticker, cost, shares) VALUES (%s, %s, %s)",
                        (p['ticker'], p['cost'], p['shares']))
        conn.commit()
    ...
    if new_tickers:
        with get_db_connection() as conn:
            ...
            for t in new_tickers:
                cur.execute(
                    "INSERT INTO user_watchlist (ticker, market) VALUES (%s, %s)", (t, market))
            conn.commit()
...
def update_all_gex_screener():
    ...
    results.sort(key=lambda x: abs(x.get("net_gex", 0)) if x.get(
        "net_gex") is not None else 0, reverse=True)
    ...
    for ticker in tickers:
        try:
            ...
            try:
                if tk.options:
                    ...
                    if not opt.calls.empty:
                        current_price = tk.info.get(
                            'regularMarketPrice', tk.history(period='1d')['Close'].iloc[-1])
                        idx = (opt.calls['strike'] -
                               current_price).abs().idxmin()
                        atm_strike = opt.calls.loc[idx, 'strike']
                        call_price = opt.calls[opt.calls['strike'] == atm_strike]['lastPrice'].values[0] if len(
                            opt.calls[opt.calls['strike'] == atm_strike]) > 0 else 0
                        put_price = opt.puts[opt.puts['strike'] == atm_strike]['lastPrice'].values[0] if len(
                            opt.puts[opt.puts['strike'] == atm_strike]) > 0 else 0
            ...
            earnings_data[ticker] = {
                "date": None, "expected_move": None, "last_updated": time.time()}
...
def generate_tactical_advice(current_price, call_wall, put_wall, gamma_flip, ma_trend, vwap_status, ai_signal=None, unusual_options=None):
    ...
    if call_wall is not None:
        ...
        if dist < 1 and dist > 0:
            advice.append(
                f"🔴 股價距離 Call Wall ${call_wall:.2f} 僅 {dist:.2f}%，面臨強壓力")
        elif dist < 0:
            advice.append(f"🚨 突破 Call Wall ${call_wall:.2f}，多頭強勢")
        else:
            advice.append(f"📈 低於 Call Wall {dist:.2f}%，上行空間充足")
    else:
        advice.append("📊 Call Wall 暫無")
    if put_wall is not None:
        dist = (current_price - put_wall) / current_price * 100
        if dist < 1 and dist > 0:
            advice.append(
                f"🟢 接近 Put Wall ${put_wall:.2f}，強支撐")
        elif dist < 0:
            advice.append(f"⚠️ 跌破 Put Wall ${put_wall:.2f}，加速下跌風險")
        else:
            advice.append(f"📉 高於 Put Wall {dist:.2f}%，支撐較遠")
    else:
        advice.append("📊 Put Wall 暫無")
    if gamma_flip is not None:
        if current_price > gamma_flip:
            advice.append(
                f"⚡ 站上 Gamma Flip ${gamma_flip:.2f}，做市商撐盤")
        else:
            advice.append(f"⚡ 低於 Gamma Flip ${gamma_flip:.2f}，易加速下跌")
    else:
        advice.append("📊 Gamma Flip 暫無")
    if ma_trend == "多頭排列":
        advice.append("🐂 均線多頭排列，中期強勢")
    elif ma_trend == "空頭排列":
        advice.append("🐻 均線空頭排列，反彈賣點")
    if vwap_status != "N/A":
        if "站上" in vwap_status:
            advice.append("📊 站上 VWAP")
        else:
            advice.append("📊 跌破 VWAP")
    if ai_signal:
...
def generate_trading_suggestion(ticker, market, current_price, gex_data, delta_analysis, pcr, ma_trend):
    ...
    if high_delta_count >= 2:
        reasons.append(
            f"多個高 Delta 期權爆量（{', '.join(delta_strikes[:2])}），大戶積極看多")
        is_bullish = True
...
def calculate_option_delta(ticker, expiration_date, strike, option_type='call'):
    try:
        ...
        if len(hist_data) < 2:
            ...
            sigma = hist_data['Close'].pct_change(
    ...
    try:
        ...
        for _, row in opt.calls.iterrows():
            ...
            if oi > 0 and vol / oi > 3:
                unusual.append(
                    f"Call ${row['strike']} 爆量 (Vol/OI={vol/oi:.1f})")
        for _, row in opt.puts.iterrows():
            vol = row['volume'] if not pd.isna(row['volume']) else 0
            oi = row['openInterest'] if not pd.isna(row['openInterest']) else 0
            if oi > 0 and vol / oi > 3:
                unusual.append(
                    f"Put ${row['strike']} 爆量 (Vol/OI={vol/oi:.1f})")
        return unusual[:3]
    except:
        return []
...
def gex_plot(ticker):
    ...
    try:
        ...
        update_time_str = datetime.fromtimestamp(cache_time).strftime(
            '%Y-%m-%d %H:%M:%S') if cache_time else "剛剛"
        ...
        same_wall_flip = (call_wall and gamma_flip and abs(
            call_wall - gamma_flip) < 0.01)
        flip_highlight = "⭐️ 關鍵轉折點" if same_wall_flip else ""
        if current_price:
            call_dist_pct = ((call_wall - current_price) /
                             current_price * 100) if call_wall else None
            put_dist_pct = ((put_wall - current_price) /
                            current_price * 100) if put_wall else None
            flip_dist_pct = ((gamma_flip - current_price) /
                             current_price * 100) if gamma_flip else None
        else:
        ...
        fig.add_trace(go.Scatter(x=strikes, y=cumulative_gex, mode='lines+markers',
                                 ...
                                 line=dict(color='#ffaa00',
                                           width=3, dash='solid'),
                                 marker=dict(
                                     size=6, color='#ffaa00', symbol='circle'),
        ...
        used_pos = {'bottom': False, 'top': False,
        ...
        fig.update_layout(
            ...
            yaxis=dict(tickfont=dict(
                size=10), range=[-max_abs, max_abs], zeroline=True, zerolinecolor='#ffffff', zerolinewidth=2)
        )
...
def add_batch_tw():
    default_tickers = ["2330", "2317", "2454", "2308",
                       "2412", "2881", "2882", "2891", "1303", "1326"]
    tickers_str = request.form.get("tickers", ",".join(default_tickers))
    tickers = [t.strip().upper() for t in tickers_str.replace(
        '\n', ',').replace(' ', ',').split(',') if t.strip()]
    added, new = add_multiple_stocks(tickers, 'tw')
...
def delete_tw(ticker):
    with get_db_connection() as conn:
        cur = conn.cursor()
        cur.execute(
            "DELETE FROM user_watchlist WHERE ticker = %s AND market = 'tw'", (ticker.upper(),))
        conn.commit()
...
def delete_us(ticker):
    with get_db_connection() as conn:
        cur = conn.cursor()
        cur.execute(
            "DELETE FROM user_watchlist WHERE ticker = %s AND market = 'us'", (ticker.upper(),))
        conn.commit()
...
def gex_screener():
    ...
    for item in data:
        if "last_update" in item:
            item["last_update_str"] = datetime.fromtimestamp(
...
def battle_map():
    try:
        with open(GEX_SCREENER_FILE, 'r', encoding='utf-8') as f:
            ...
            gex_data = {
                item['ticker']: item for item in gex_items}
    except:
        gex_data = {}
    try:
        with open(AI_SCORES_FILE, 'r', encoding='utf-8') as f:
            ...
            ai_data = {
                item['ticker']: item for item in ai_items if item.get('market') == 'us'}
    except:
        ai_data = {}
    stocks_info = []
    for ticker in get_all_tickers('us'):
        ...
        vwap_status = "站上 VWAP" if ai.get('vwap_status') and "站上" in ai.get(
            'vwap_status', '') else "跌破 VWAP" if ai.get('vwap_status') else "N/A"
        advice = generate_tactical_advice(
            curr_price, call_wall, put_wall, gamma_flip, ma_trend, vwap_status)
    ...
    stocks_info.sort(key=lambda x: x.get('net_gex', 0) if x.get(
        'net_gex') is not None else 0, reverse=True)
...
def index():
    try:
        all_scores = json.load(open(AI_SCORES_FILE, 'r', encoding='utf-8'))
    except:
        all_scores = []
    tw_stocks = [s for s in all_scores if s.get('market') == 'tw'][:10]
    us_stocks = [s for s in all_scores if s.get('market') == 'us'][:10]
    try:
        alerts = json.load(open(ALERTS_FILE, 'r', encoding='utf-8'))[:10]
    except:
        alerts = []
...
def tenbagger_radar_page():
    try:
        candidates = json.load(open(TENBAGGER_FILE, 'r', encoding='utf-8'))
    except:
        candidates = []
...
def backtest_page(ticker):
    ...
    result = run_backtest_advanced(
        ticker, days=180, benchmark_ticker=benchmark)
    if result is None:
        return render_template('backtest.html', ticker=ticker, error="資料不足，無法回測")
...
def settings_page():
    if request.method == 'POST':
        settings['max_loss_per_trade'] = float(
            request.form.get('max_loss_per_trade', 5))
        settings['max_concentration'] = float(
            request.form.get('max_concentration', 40))
        settings['cash_balance'] = float(request.form.get('cash_balance', 0))
        settings['bonding_threshold'] = float(
            request.form.get('bonding_threshold', 3))
        settings['background_update_minutes'] = int(
            request.form.get('background_update_minutes', 15))
        settings['cache_ttl_minutes'] = int(
            request.form.get('cache_ttl_minutes', 5))
        save_settings(settings)
        watchlist = request.form.get('watchlist', '').upper()
        if watchlist:
            with open(WATCHLIST_FILE, 'w', encoding='utf-8') as f:
                json.dump([t.strip()
                          for t in watchlist.split(',') if t.strip()], f)
...
def earnings_calendar():
    ...
    stocks_info.sort(key=lambda x: (
        x['date'] == "N/A" or x['date'] is None, x['date']))
        ...
        while True:
            try:
                ...
                if new_hash != _last_alerts_hash:
                    ...
                    high_alerts = [a for a in new_alerts if a.get(
                        'importance') == 'high'][:3]
                    if high_alerts:
                        msg = " | ".join(
                            [f"{a['ticker']}: {a['message']}" for a in high_alerts])
                        yield f"data: 🚨 {msg}\n\n"
                    else:
                        if new_alerts:
                            msg = " | ".join(
                                [f"{a['ticker']}: {a['message']}" for a in new_alerts[:3]])
                            yield f"data: 📢 {msg}\n\n"
    ...
    with closing(get_db_connection()) as conn:
        ...
        if DATABASE_URL:
            cur.execute('''
                ...
                ORDER BY stock_id
            ''')
            rows = [dict(zip([desc[0] for desc in cur.description], row))
...
if __name__ == "__main__":
    ...
    print(
        f"📊 訪問地址：0.0.0.0:{os.environ.get('PORT', 5000)}  (debug={debug_mode})")
